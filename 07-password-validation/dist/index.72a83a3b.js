...
function checkPasswordIsValid(password) {
    contains.forEach((item, index)=>{
        if (index == 0) password.length >= item ? listItems[index].classList.add("active") : listItems[index].classList.remove("active");
        else password.match(item) ? listItems[index].classList.add("active") : listItems[index].classList.remove("active");
    });
}
